...
pygame.display.set_caption('SNAKE KENZIE')
...
def verifica_comida(dx, dy, x_comida, y_comida, lista_cobra):
    ...
    if head[0] == x_comida and head[1] == y_comida:
        ...
        y_comida = round(random.randrange(0, 600 - d) / 20) * 20
        pygame.mixer.init()
        pygame.mixer.music.load('bite_food.mp3')
...
def show_go_screen(lista_cobra):
    self.screen.blit(amarelo, (400, 400))
    text_surface = FONT.render("Press space bar to play again", True, azul)
